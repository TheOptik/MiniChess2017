...
public class Client {
	...
	/**
	 * Get a response from the server. Blocks until response is received. Fails if
	 * the response code received does not match the response code accepted.
	 * 
	 ...
	 * 
	 * @param server   hostname or IP address of IMCS server, usually
	...
	/**
	 * Instruct the server to offer a game to other players. If color is "?", allow
	 * the other side or the server to pick the color. Blocks until the offer is
	 * accepted.
	 * 
	...
	/**
	 * Instruct the server to accept a game. If color is "?", allow the other side
	...
	public boolean hasOfferings() {
		...
		out.flush();
		try {
			in.readLine(); // ignore return code
